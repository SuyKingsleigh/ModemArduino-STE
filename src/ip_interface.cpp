...
void IpInterface::print_ip(){ 
    Serial.print(F("IP: "));      Serial.println(this->ip->get_ip_addr()); 
    Serial.print(F("Mascara: ")); Serial.println(this->ip->get_mask_addr()); 
    Serial.print(F("Gateway: ")); Serial.println(this->ip->get_gw_addr()); 
    // delay(100);
}

void IpInterface::update_ip(){ 

    Serial.println(F("Digite o novo endereco IP, deve ter o seguinte formato: xxx.xxx.xxx.xxx \n")); 
    String user_input = leStringSerial(); 


    if(this->ip->set_ip_addr_static(user_input)){
        Serial.println(F("Endereço IP trocado com sucesso, novo endereco e: \n"));
        Serial.println(this->ip->get_ip_addr());
        delay(50);
    }else{
        Serial.println(F("Endereço IP nao foi alterado, nao esta no padrao xxx.xxx.xxx.xxx  \n"));
        delay(50);    
    }
...
void IpInterface::update_mask(){ 
    ...
    Serial.println(F("Digite a nova mascara de rede, deve ter o seguinte formato: xxx.xxx.xxx.xxx \n")); 
    String user_input = leStringSerial(); 
    
    if(this->ip->set_mask_addr_static(user_input)){
        Serial.println(F("Mascara de rede trocada com sucesso, nova máscara e: \n"));
        Serial.println(this->ip->get_mask_addr());
        delay(50);
    }else{
        Serial.println(F("Mascada nao foi alterada, nao esta no padrao xxx.xxx.xxx.xxx \n"));
        delay(50);
    }
}
...
void IpInterface::update_gw(){
    ...
    Serial.println(F("Digite o novo Gateway, deve ter o seguinte formato: xxx.xxx.xxx.xxx \n")); 
    String user_input = leStringSerial(); 

    if(this->ip->set_gw_addr_static(user_input)){
        Serial.println(F("Gateway trocado com sucesso, novo Gateway e: \n"));
        Serial.println(this->ip->get_gw_addr());
        delay(50);
    }else{
        Serial.println(F("Gateway nao foi alterado, nao esta no padrao xxx.xxx.xxx.xxx \n"));
        delay(50);       
    }      
}
...
void IpInterface::update_all_fields(){ 
    ...
    if(this->ip->set_ip_addr_static(user_input1)&&this->ip->set_ip_addr_static(user_input2)&&this->ip->set_ip_addr_static(user_input3)){
        ...
        Serial.println(F("Endereco alterado, novo endereco e:  \n"));
        // Serial.println ("IP: " +this->ip->get_ip_addr()+"\n"+"Mascara: "+this->ip->get_mask_addr()+"\n"+"Gateway: "+this->ip->get_gw_addr());
        Serial.print(F("IP: ")); Serial.println(this->ip->get_ip_addr());
        Serial.print(F("Mascara: ")); Serial.println(this->ip->get_mask_addr());
        Serial.print(F("Gateway: ")); Serial.print(this->ip->get_gw_addr());
    }      
}
